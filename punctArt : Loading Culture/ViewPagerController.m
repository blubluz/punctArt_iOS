...
@property (nonatomic) NSNumber *fixedTabWidth;
...
@synthesize fixedTabWidth = _fixedTabWidth;
...
- (void)layoutSubviews {
    ...
    NSLog(@"%i", self.test);    CGRect frame = self.tabsView.frame;
    frame.origin.x = 0.0;
    ...
    frame.origin.y = 65+self.tabHeight.floatValue;
    frame.size.width = CGRectGetWidth(self.view.frame);
    frame.size.height = CGRectGetHeight(self.view.frame) - (/*topLayoutGuide +*/ CGRectGetHeight(self.tabsView.frame)) - /*CustomTabBarHeight*/ 57 ;
...
- (void)setFixedTabWidth:(NSNumber *)fixedTabWidth{
    if ([fixedTabWidth floatValue] != 1.0 && [fixedTabWidth floatValue] != 0.0)
        fixedTabWidth = [fixedTabWidth boolValue] ? [NSNumber numberWithBool:YES] : [NSNumber numberWithBool:NO];
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex {
    ...
    labelSubview.textColor = [self.delegate viewPager:self colorForComponent:ViewPagerUnselectedText withDefault:[UIColor colorWithRed:207.0/255.0 green:207.0/255.0 blue:207.0/255.0 alpha:1]];
    ...
    labelSubview.textColor = [self.delegate viewPager:self colorForComponent:ViewPagerSelectedText withDefault:[UIColor colorWithRed:51.0/255.0 green:51.0/255.0 blue:51.0/255.0 alpha:1]];
...
-(NSNumber *)fixedTabWidth {
    if(!_fixedTabWidth) {
        CGFloat value = kFixedTabWidth;
        if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
            value = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabFixedWidth withDefault:value];
        self.fixedTabWidth = [NSNumber numberWithFloat:value];
    }
    return _fixedTabWidth;
}
- (NSNumber *)tabOffset {
...
- (void)setNeedsReloadOptions {
    ...
    //self.leftOffset = [NSNumber numberWithFloat:[self.delegate viewPager:ViewPagerOptionTabOffset valueForOption:<#(ViewPagerOption)#> withDefault:<#(CGFloat)#>]]
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++) {
        ...
        //frame.size.width = tabView.frame.size.width+12.0;
        tabView.frame = frame;
...
- (CGFloat)valueForOption:(ViewPagerOption)option {
    ...
    switch (option) {
        ...
        case ViewPagerOptionTabFixedWidth:
            return [[self fixedTabWidth] floatValue];
        default:
            return NAN;
    }
}
...
- (TabView *)tabViewAtIndex:(NSUInteger)index {
    ...
    if ([[self.tabs objectAtIndex:index] isEqual:[NSNull null]]) {
        ...
        if(![self.fixedTabWidth boolValue])
            self.tabWidth = [NSNumber numberWithFloat:tabViewContent.frame.size.width+30];
